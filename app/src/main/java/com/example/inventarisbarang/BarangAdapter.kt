...
class BarangAdapter(
    ...
    inner class BarangViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val namaTextView: TextView =
            itemView.findViewById(R.id.nama_text_view) // TextView untuk nama barang
        val kategoriTextView: TextView =
            itemView.findViewById(R.id.kategori_text_view) // TextView untuk kategori barang
        val jumlahTextView: TextView =
            itemView.findViewById(R.id.jumlah_text_view) // TextView untuk jumlah barang
        val buttonDelete: Button =
            itemView.findViewById(R.id.button_delete) // Tombol untuk menghapus barang
        val buttonEdit: Button =
            itemView.findViewById(R.id.button_edit) // Tombol untuk mengedit barang
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BarangViewHolder {
        ...
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.barang_item, parent, false)
    ...
    override fun onBindViewHolder(holder: BarangViewHolder, position: Int) {
        ...
        holder.kategoriTextView.text =
            "Kategori: ${currentBarang.kategori}" // Mengatur kategori barang
        holder.jumlahTextView.text =
            "Jumlah: ${currentBarang.jumlah} Unit" // Mengatur jumlah barang
...
//    @SuppressLint("NotifyDataSetChanged") // Menonaktifkan peringatan karena menggunakan notifyDataSetChanged()
    internal fun setBarang(barangs: List<Barang>) {
        val oldList = this.barangList
        this.barangList = barangs // Mengatur daftar barang yang baru

        for (i in barangs.indices) {
            if (oldList.size > i) {
                if (oldList[i] != barangs[i]) {
                    notifyItemChanged(i)
                }
            } else {
        ...
        if (oldList.size > barangs.size) {
            for (i in barangs.size until oldList.size) {
                notifyItemRemoved(i)
            }
        }
    }
